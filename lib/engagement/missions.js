...
/** Lazy-load Mongoose models so requiring this module does not touch Mongo (e.g. platformPlay unit tests). */
function getModels() {
    ...
    if (!syncPromise) {
        syncPromise = (async () => {
            for (const d of DEFAULT_MISSIONS) {
                await getModels().MissionDefinition.findOneAndUpdate({ missionKey: d.missionKey }, { $set: d }, { upsert: true });
            }
        })();
    }
    await syncPromise;
...
async function upsertMissionProgress(filter, def, patch) {
    await getModels().MissionProgress.findOneAndUpdate(
        filter,
        {
            $set: {
                ...patch,
...
async function applyMissionIncrement(filter, def, gameTag, event) {
    const doc = await getModels().MissionProgress.findOne(filter);
...
async function onPlatformMissionHook(p) {
    ...
    const defs = await getModels().MissionDefinition.find({ objectiveType: { $ne: 'duelWins' } }).lean();
...
async function onDuelMissionHook(p) {
    ...
    await ensureMissionDefinitionsSynced();
    const defs = await getModels().MissionDefinition.find({ objectiveType: 'duelWins' }).lean();
    const now = new Date();
    for (const def of defs) {
...
async function grantMissionReward(guildId, userId, def) {
    if (def.rewardType === 'credits') {
        await getModels().User.findOneAndUpdate({ guildId, userId }, { $inc: { points: def.rewardAmount } });
    } else if (def.rewardType === 'season_xp') {
        await getModels().EngagementProfile.findOneAndUpdate(
            { guildId, userId },
...
async function claimCompletedMissions(guildId, userId, factionName) {
    ...
    const userRows = await getModels().MissionProgress.find({
        guildId,
        userId,
        scope: 'user_daily',
        periodKey: day,
    ...
    for (const row of userRows) {
        const def = await getModels().MissionDefinition.findOne({ missionKey: row.missionKey }).lean();
        if (!def) continue;
        await grantMissionReward(guildId, userId, def);
        await getModels().MissionProgress.updateOne({ _id: row._id }, { $set: { claimed: true } });
        total++;
        lines.push(`• **${def.title}** → **${def.rewardAmount}** ${def.rewardType.replace('_', ' ')}`);
    }

    if (factionName) {
        const facRows = await getModels().MissionProgress.find({
            guildId,
            factionName,
            userId: null,
        ...
        for (const row of facRows) {
            ...
            if (!def) continue;
            await grantMissionReward(guildId, userId, def);
            await getModels().MissionProgress.updateOne({ _id: row._id }, { $push: { claimedByUserIds: userId } });
            total++;
...
async function listMissionBoard(guildId, userId, factionName) {
    ...
    const day = utcDayKey(now);
    const week = utcWeekKey(now);
    const defs = await getModels().MissionDefinition.find().sort({ scope: 1, missionKey: 1 }).lean();
    const lines = [];
    ...
    for (const def of defs) {
        ...
        let row = null;
        if (def.scope === 'user_daily') {
            row = await getModels().MissionProgress.findOne({
                guildId,
                userId,
                factionName: null,
                periodKey,
...
async function listMissionDefinitionsLean() {
    await ensureMissionDefinitionsSynced();
    return getModels().MissionDefinition.find().sort({ scope: 1, missionKey: 1 }).lean();
}
