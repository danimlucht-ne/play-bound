...
const { DuelProfile } = require('../../models');
...
async function recordDuelOutcome(guildId, winnerId, loserId) {
    const wDoc = await DuelProfile.findOne({ guildId, userId: winnerId });
    const lDoc = await DuelProfile.findOne({ guildId, userId: loserId });
    ...
    await DuelProfile.findOneAndUpdate(
