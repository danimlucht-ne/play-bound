...
async function recordFactionChallengePoints({ client, guildId, userId, factionName, points, gameTag }) {
    ...
    // Use atomic $inc to prevent concurrent score overwrites
    const incFields = {};
    incFields[`rawScoresByUser.${userId}`] = points;
    incFields[`scoresByUser.${userId}`] = points;
    ...
    await tryFinalizeChallengeOnPointCap(client, guildId, updated);
    return {
        credited: true,
