...
async function recordFactionChallengePoints({ client, guildId, userId, factionName, points, gameTag }) {
    ...
    const phase = getWarPhase(ch, new Date());
    if (phase === 'prep') {
        playboundDebugLog(
            `[faction-war-skip] guild=${guildId} user=${userId} tag=${gameTag} reason=${FactionCreditReasonCode.PREP_WINDOW}`,
        );
        return {
            credited: false,
            reasonCode: FactionCreditReasonCode.PREP_WINDOW,
            userMessage: 'This war is still in **prep** — scoring opens when the prep timer ends.',
        };
    }
    ...
    if (isChallengeRanked(ch) && Array.isArray(ch.warPointLedger) && ch.warPointLedger.length < MAX_WAR_LEDGER_ENTRIES) {
        ch.warPointLedger.push({
            at: new Date(),
            userId: String(userId),
            gameTag: String(gameTag).toLowerCase(),
            counted: addCounted,
            raw: Math.max(0, Math.floor(Number(points) || 0)),
        });
        ch.markModified('warPointLedger');
    }
...
function computeTeamValues(challenge) {
    const { teams, label } = computeOfficialTeamValues(challenge, { getScoreByUser });
    return { teams, label };
}
