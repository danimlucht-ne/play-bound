...
const { pickRankedFeaturedTags: pickRankedFeaturedFromPool } = require('./rankedFeaturedPick');
...
async function ensureRotationForDate(date = new Date()) {
    ...
    if (existing && existing.activeTags && existing.activeTags.length) {
        return {
            ...
            rankedFeaturedTags: existing.rankedFeaturedTags || [],
            fromManual: false,
        };
    }
    ...
    if (Array.isArray(settings.manualActiveTags) && settings.manualActiveTags.length) {
        ...
        const rankedFeaturedTags = pickRankedFeaturedFromPool(dayUtc, activeTags, (t) => resolveGame(t, settings));
        ...
        return { dayUtc, activeTags, featuredTag, rankedFeaturedTags, fromManual: true };
    }
    ...
    return { dayUtc, activeTags, featuredTag, rankedFeaturedTags, fromManual: false };
}
...
async function setManualDayOverride(dayUtc, activeTags, featuredTag) {
    ...
    await GamePlatformDay.findOneAndUpdate(
        { dayUtc },
        { $set: { activeTags, featuredTag, rankedFeaturedTags, computedAt: new Date() } },
        { upsert: true },
    );
}
