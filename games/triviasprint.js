...
} = require('../lib/triviaSprintQuestionTimer');
...
async function triggerTriviaSprintEnd(client, threadId) {
    ...
    clearAllSprintQuestionTimers(activeSprint);
...
async function handleTriviaSprintButton(interaction, client) {
    ...
    if (interaction.customId === 'sprint_start') {
        ...
        clearSprintQuestionTimer(activeSprint, interaction.user.id);
        if (!player) {
        ...
        scheduleSprintQuestionTimer(client, activeSprints, activeSprint.threadId, interaction.user.id, interaction);
        return true;
    }
